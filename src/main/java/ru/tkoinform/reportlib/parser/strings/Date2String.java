...
/**
 * Реализация парсера в строку для <code>RLReportColumn.DATE</code>
 ...
 * <p>
 * На вход <code>SRC</code> подается любой объект типа дата, который мы готовы преобразовать в строку
 * <p>
 * <p>
 * Может показаться что это избыточный класс, и делает тоже самое, что <code>String2String</code> с типизацией по
 * дате, однако архитектура такова, что отображать типизированный в дату RLReportColumn мы можем иначе,
 * чем преобразование в строку по умолчанию, в том числе используя дополнительые аттрибуты самой колонки
 * <p>
 * Здесь мы применяем более тонкие настройки RSColumn относящиеся к дате.
 */
public class Date2String<SRC> extends AbstractAny2Any<SRC, String> {


    public Date2String(RLReportColumn column, ReportFormat reportFormat, Class<SRC> resultSetClass) {
        super(column, reportFormat, resultSetClass);
    }

    @Override
    protected Any2Any<SRC, String> createInternalFormatter() {
        if (ClassUtils.isDate(resultSetClass) || ClassUtils.isDateTime(resultSetClass)) {
